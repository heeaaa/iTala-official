...
(async () => {
  ...
  try {
    ...
    await db.exec(section('create or replace function public.rec_setup_game(', 'grant execute on function public.rec_setup_game(text,text,boolean,bigint,text,text,boolean,boolean,jsonb) to authenticated;'));
    const addition = section('-- BEGIN REC SETUP RECEIPTS', '-- END REC SETUP RECEIPTS');
    await db.exec(addition);
    await db.exec(read('tests/sql/rec_setup_receipts.test.sql'));
    ...
    assert.equal((await db.query('select count(*)::int as n from public.rec_setup_receipts')).rows[0].n, 2);
    console.log('PASS: PostgreSQL drop-in receipts suite and schema reapplication preserve existing data.');

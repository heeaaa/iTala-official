...
- [ ] **`delete-account` Edge Function deployed, with its four Apple secrets set.** Sign in with Apple works without it, so this is easy to miss - but App Review 5.1.1(v) requires deletion to **revoke** the Apple authorization, and that needs an ES256 client secret signed with a Sign in with Apple private key (a different Apple credential from the provider setup above). Commands and secret names: `supabase/functions/README.md`. Until it is deployed, Apple-linked deletion is refused with a readable error rather than deleting anything - the safe failure, but still a broken flow
- [ ] **Apple deletion revocation verified on a device** (R57 in `tests/MANUAL-REGRESSION.md`): after deleting, iTala is **gone** from *Settings → your name → Sign-In & Security → Sign in with Apple*. `npm test` uses a fake Apple and cannot prove this. Use a development or TestFlight build, not Expo Go - Apple signs an Expo Go credential for `host.exp.Exponent`, which these secrets cannot revoke
- [ ] Deletion disclosure re-read on all three surfaces so they agree: the in-app confirmation, privacy policy section 10, and the App Review notes in `docs/APP_REVIEW.md`. All three must say that a previously submitted content report and its app-session identifier may remain (`node tests/run.js` checks their presence, not their accuracy)
